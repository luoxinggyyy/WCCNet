...
class LevelWCCNet(nn.Module):
    def __init__(self,in_planes, kernel_size, dropout, groups, hidden_size, INN):
        super(LevelWCCNet, self).__init__()
...
class WCCNet_Tree(nn.Module):
    def __init__(self, in_planes, current_level, kernel_size, dropout, groups, hidden_size, INN):
        ...
        if current_level!=0:
            self.WCCNet_Tree_odd=WCCNet_Tree(in_planes, current_level-1, kernel_size, dropout, groups, hidden_size, INN)
            self.WCCNet_Tree_even=WCCNet_Tree(in_planes, current_level-1, kernel_size, dropout, groups, hidden_size, INN)
    ...
    def forward(self, x):
        ...
        if self.current_level ==0:
            ...
            return self.zip_up_the_pants(self.WCCNet_Tree_even(x_even_update), self.WCCNet_Tree_odd(x_odd_update))

class EncoderTree(nn.Module):
    def __init__(self, in_planes,  num_levels, kernel_size, dropout, groups, hidden_size, INN):
        super().__init__()
        self.levels=num_levels
        self.WCCNet_Tree = WCCNet_Tree(
            in_planes = in_planes,
...
if __name__ == '__main__':
    ...
    model = WCCNet(output_len = args.horizon, input_len= args.window_size, input_dim = 9, hid_size = args.hidden_size, num_stacks = 1,
