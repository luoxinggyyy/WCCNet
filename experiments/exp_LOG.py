...
from models.WCCNet import WCCNet
from models.WCCNet_decompose import WCCNet_decompose

class exp_LOG(Exp_Basic):
    def __init__(self, args):
        super(exp_LOG, self).__init__(args)
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(valid_loader):
            pred, pred_scale, mid, mid_scale, true, true_scale = self._process_one_batch_WCCNet(
        ...
        if self.args.stacks == 1:
            ...
            print('normed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, lxmspe:{:.4f}, corr:{:.4f}'.format(mse, mae, rmse, mape, mspe, corr))
            print('denormed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, lxmspe:{:.4f}, corr:{:.4f}'.format(mses, maes, rmses, mapes, mspes, corrs))
        elif self.args.stacks == 2:
            preds = np.array(preds)
            trues = np.array(trues)
            mids = np.array(mids)
    ...
    def test(self, setting, evaluate=False):
        ...
        if self.args.stacks == 1:
            ...
            if preds.ndim == 2:
                predd = np.squeeze(preds,axis=2)
                truee = np.squeeze(trues,axis=2)
                turelx = pd.DataFrame(truee)
                predlx = pd.DataFrame(predd)         
                turelx.to_csv("ture12.csv")
                predlx.to_csv("pred12.csv")
            else:
                predss = preds[:,0,:]
                truess = trues[:,0,:]
                turelx = pd.DataFrame(truess)
                predlx = pd.DataFrame(predss)
                turelx.to_csv("ture11.csv")
                predlx.to_csv("pred11.csv")


            mae, mse, rmse, mape, mspe, corr = metric(preds, trues)
            maes, mses, rmses, mapes, mspes, corrs = metric(pred_scales, true_scales)
            print('normed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}, lxcorr:{:.4f}'.format(mse, mae, rmse, mape, mspe, corr))
            print('TTTT denormed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}, lxcorr:{:.4f}'.format(mses, maes, rmses, mapes, mspes, corrs))
            ...
            mae, mse, rmse, mape, mspe, corr = metric(mids, trues)
            maes, mses, rmses, mapes, mspes, corrs = metric(mid_scales, true_scales)
            print('Mid --> normed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}, lxxcorr:{:.4f}'.format(mse, mae, rmse, mape, mspe, corr))

            mae, mse, rmse, mape, mspe, corr = metric(preds, trues)
            maes, mses, rmses, mapes, mspes, corrs = metric(pred_scales, true_scales)
            print('TTTT Final --> denormed mse:{:.4f}, mae:{:.4f}, rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}, lxxcorr:{:.4f}'.format(mse, mae, rmse, mape, mspe, corr))
    ...
    def _process_one_batch_WCCNet(self, dataset_object, batch_x, batch_y):
        batch_x = batch_x.double().cuda()
        batch_y = batch_y.double()

        if self.args.stacks == 1:
            outputs = self.model(batch_x)  ##这个
